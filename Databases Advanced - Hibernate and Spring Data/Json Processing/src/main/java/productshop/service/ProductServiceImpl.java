...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public List<SuccessfullySoldProductsDto> successfullySoldProducts() {
        List<Product> entities = this.productRepository.extractAllSuccessfullySoldProducts();
        List<SuccessfullySoldProductsDto> dtos = new ArrayList<>();

        for (Product entity : entities) {

            boolean containsSeller = false;

            for (SuccessfullySoldProductsDto dto : dtos) {
                if (dto.getFirstName().equals(entity.getSeller().getFirstName()) &&
                        dto.getLastName().equals(entity.getSeller().getLastName())) {

                    BuyerProductDto buyerProductDto = modelMapper.map(entity, BuyerProductDto.class);
                    buyerProductDto.setFirstName(entity.getBuyer().getFirstName());
                    buyerProductDto.setLastName(entity.getBuyer().getLastName());
                    dto.getSoldProducts().add(buyerProductDto);
            ...
            if (!containsSeller) {
                SuccessfullySoldProductsDto newDto = modelMapper.map(entity.getSeller(), SuccessfullySoldProductsDto.class);
                BuyerProductDto buyerProductDto = modelMapper.map(entity, BuyerProductDto.class);
                buyerProductDto.setFirstName(entity.getBuyer().getFirstName());
                buyerProductDto.setLastName(entity.getBuyer().getLastName());
                newDto.getSoldProducts().add(buyerProductDto);
